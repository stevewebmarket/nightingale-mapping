...
def get_sequence_from_melody(sound, num_notes, tolerance=0.08):
    """Improved decoder that returns Fraction pairs"""
...
for seq in [[1,3,1], [2,3,3,2]]:
    ...
    print(f"  {seq} mirrored → {decoded} (symmetric: {decoded[:len(seq)] == decoded[len(seq):][::-1]})")
